...
# ============================================================
# SA-PAFT: Sensitivity-Aware Prompt-Agnostic Fine-Tuning
# ============================================================

def parse_example_content(content):
    """Parse HellaSwag example string into template variables."""
    try:
        if "A." in content and "B." in content:
            ctx = content.split("A.")[0].strip()
            if "Question:" in ctx:
                ctx = ctx.split("Question:")[0].strip()
            ab = content.split("A.")[1]
            A = ab.split("B.")[0].strip()
            bc = ab.split("B.")[1]
            B = bc.split("C.")[0].strip() if "C." in bc else bc.strip()
            if "C." in content:
                cd = content.split("C.")[1]
                C = cd.split("D.")[0].strip() if "D." in cd else cd.strip()
            else:
                C = ""
            if "D." in content:
                D = content.split("D.")[1].split("You may")[0].split("Answer:")[0].strip()
            else:
                D = ""
            return {"ctx": ctx, "A": A, "B": B, "C": C, "D": D}
    except Exception:
        return None
    return None


def compute_centroid_jsd(model, tokenizer, prompt_templates, example_content, device, num_variants=8):
    """
    SA-PAFT core signal: multi-distribution Jensen-Shannon Divergence.
    High score = model output changes across prompts = needs more training.
    Low score  = model already robust for this example.
    """
    import torch
    import torch.nn.functional as F
    model.eval()
    sampled = random.sample(prompt_templates, min(num_variants, len(prompt_templates)))
    distributions = []

    with torch.no_grad():
        for template in sampled:
            try:
                filled = template.format(**example_content)
                inputs = tokenizer(
                    filled, return_tensors="pt",
                    truncation=True, max_length=512
                ).to(device)
                outputs = model(**inputs)
                dist = F.softmax(outputs.logits[0, -1, :], dim=-1).cpu().float()
    ...
    dists = torch.stack(distributions)
    centroid = dists.mean(dim=0)
...
def compute_sensitivity_weights(model, tokenizer, all_datasets,
                                prompt_templates, device,
                                temperature=1.0, sample_size=100):
    """
    Computes per-example sensitivity weights using centroid JSD.
    Called at the start of each epoch from epoch 2 onwards.
    Returns sampling weights array for WeightedRandomSampler.
    """
    import json as _json
    print(f"[SA-PAFT] Computing sensitivity weights on {sample_size} examples...")
    total = len(all_datasets)
    scores = np.zeros(total)

    # Training dataset is tokenized so load raw JSON directly
    # Try to find the file robustly relative to workspace or hardcoded fallback
    possible_paths = [
        'data/hellaswag_train.json',
        'PAFT/data/hellaswag_train.json',
        'd:/PAFT/PAFT/data/hellaswag_train.json',
        '/content/PAFT_new/PAFT/data/hellaswag_train.json'
    ]
    raw_path = next((p for p in possible_paths if os.path.exists(p)), possible_paths[0])
    try:
        with open(raw_path) as f:
            raw_data = _json.load(f)
        print(f"[SA-PAFT] Loaded {len(raw_data)} raw examples")
    except Exception as e:
        print(f"[SA-PAFT] Could not load raw data: {e}")
        weights = np.ones(total) / total
        return weights, scores

    eval_size = min(sample_size, len(raw_data), total)
    eval_indices = np.random.choice(eval_size, size=eval_size, replace=False)

    for idx in eval_indices:
        try:
            content = raw_data[int(idx)]['instruction']
            example_content = parse_example_content(content)
            if example_content is None:
                continue
            target_idx = int(idx) % total
            scores[target_idx] = compute_centroid_jsd(
                model, tokenizer, prompt_templates, example_content, device
            )
        except Exception:
            continue

    filled_mask = scores > 0
    mean_score = float(scores[filled_mask].mean()) if filled_mask.any() else 0.0
    scores[~filled_mask] = mean_score

    print(f"[SA-PAFT] Scores — mean: {mean_score:.4f}, max: {scores.max():.4f}, min: {scores.min():.4f}")

    weights = np.exp(scores / (temperature + 1e-8))
