...
class SensitivitySampler(torch.utils.data.Sampler):
    """
    SA-PAFT: A sampler that allows in-place updates to its weights mid-training.
    This enables dynamic weighted sampling for epochs 2+ without recreating the DataLoader.
    """
    def __init__(self, data_source, weights=None, replacement=True, num_samples=None):
        self.data_source = data_source
        self.replacement = replacement
        self._num_samples = num_samples
        
        if weights is None:
            self.weights = torch.ones(len(data_source), dtype=torch.float)
        else:
    ...
    def update_weights(self, new_weights):
        """Update the sampler's multinomial weights in-place."""
        self.weights = torch.as_tensor(new_weights, dtype=torch.float)

    def __iter__(self):
        # Standard WeightedRandomSampler behavior: re-draw indices for the epoch.
...
class PromptSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def __init__(
        ...
        self.processor = processor
        # SA-PAFT state
        self.sa_epoch_count = 0
        self.sa_scores_history = []
        self.sa_sampler = None
        self.prompt_templates = []
        
        # Pre-load prompts for efficiency
        if finetuning_args.prompt_search_space:
            try:
                import pandas as pd
                df = pd.read_excel(finetuning_args.prompt_search_space)
                self.prompt_templates = df["text"].tolist()
                print(f"[SA-PAFT] Loaded {len(self.prompt_templates)} prompt variants from search space.")
            except Exception as e:
                print(f"[SA-PAFT] Warning: Failed to load prompt search space: {e}")

        # SA-PAFT: Add callback to update weights at the end of each epoch.
        # This occurs before the next epoch's DataLoader iterator is created.
        if getattr(self.finetuning_args, 'use_sa_paft', False):
            class SAPAFTCallback(TrainerCallback):
                def on_epoch_end(_myself, args, state, control, **cb_kwargs):
                    self.update_sa_weights()
            self.add_callback(SAPAFTCallback())

    def update_sa_weights(self):
        """SA-PAFT: Compute sensitivity weights and update the sampler in-place."""
        self.sa_epoch_count += 1
        print(f"\n[SA-PAFT] Finished epoch {self.sa_epoch_count} — computing weights for epoch {self.sa_epoch_count + 1}...")
        try:
            from llamafactory.data.loader import compute_sensitivity_weights
            import pandas as pd, json, os

            if not self.prompt_templates:
                df = pd.read_excel(self.finetuning_args.prompt_search_space)
                self.prompt_templates = df["text"].tolist()

            weights, scores = compute_sensitivity_weights(
                model=self.model,
                tokenizer=self.tokenizer,
                all_datasets=self.train_dataset,
            ...
            self.sa_scores_history.append({
                "epoch": self.sa_epoch_count,
            ...
            if self.sa_sampler is not None:
                self.sa_sampler.update_weights(weights)
                print(f"[SA-PAFT] ✓ Sampler weights updated: mean={scores.mean():.4f}, max={scores.max():.4f}")
            else:
                print("[SA-PAFT] WARNING: sa_sampler is None, weights ignored.")

            scores_path = os.path.join(self.args.output_dir, "sa_scores.json")
            with open(scores_path, "w") as f:
                json.dump(self.sa_scores_history, f, indent=2)

        except Exception as e:
            import traceback
            print(f"[SA-PAFT] ERROR in update_sa_weights: {type(e).__name__}: {e}")
            traceback.print_exc()

    def get_train_dataloader(self) -> DataLoader:
        """
        SA-PAFT: Return a DataLoader with our custom SensitivitySampler.
        """
        ...
        if not isinstance(train_dataset, torch.utils.data.IterableDataset):
            if getattr(self.finetuning_args, 'use_sa_paft', False):
                print("[SA-PAFT] Initializing SensitivitySampler for Epoch 1")
                self.sa_sampler = SensitivitySampler(train_dataset)
                dataloader_params["sampler"] = self.sa_sampler
            else:
    ...
    def training_step(self, model, inputs):
        """SA-PAFT: Compute CE + KL loss for robust tuning."""
        if not getattr(self.finetuning_args, 'use_robust_tuning', False):
        ...
        try:
            ...
            # 1. Compute normal CE loss
            model.train()
            ce_loss, outputs1 = self.compute_loss(model, inputs, return_outputs=True)

            # 2. Decode inputs[0] and parse content
            input_ids = inputs["input_ids"][0]
            labels = inputs["labels"][0] if "labels" in inputs else None
            
            # [CRITICAL FIX] do NOT skip special tokens, otherwise ChatML markers like <|im_start|> are lost
            decoded = self.tokenizer.decode(input_ids, skip_special_tokens=False)
            
            user_content = ""
            if "<|im_start|>user" in decoded and "<|im_end|>" in decoded:
                user_content = decoded.split("<|im_start|>user")[1].split("<|im_end|>")[0].strip()
            
            parsed = parse_example_content(user_content)
            
            if parsed and self.prompt_templates:
                if not getattr(self, "sa_kl_logged", False):
                    print(f"[SA-PAFT] ✓ Robust KL Loss is now ACTIVE.")
                    self.sa_kl_logged = True

                # 3. Algorithm 1: Update prompt every K=4 steps (Section 5.3 default)
                if (self.state.global_step % 4 == 0) or (not hasattr(self, "current_sa_prompt")):
                    self.current_sa_prompt = random.choice(self.prompt_templates)
                template = self.current_sa_prompt
                filled = template.format(**parsed)
                
                # 4. Tokenize second pass (Prompt only)
                inputs2 = self.tokenizer(
                    filled, return_tensors="pt", truncation=True, max_length=512
                ).to(self.model.device)
                
                # 5. Second forward pass (no_grad)
                with torch.no_grad():
                    outputs2 = self.model(**inputs2)
                
                # 6. Correct Dist1 calculation: index where model starts predicting the answer
                # In standard SFT, inputs["labels"] has IGNORE_INDEX for instruction, and token ids for answer.
                # Predictor at index i-1 predicts token at index i.
                dist1 = None
                if labels is not None:
                    # Find first non-ignore index
                    label_indices = (labels != IGNORE_INDEX).nonzero()
                    if len(label_indices) > 0:
                        pred_idx = label_indices[0].item() - 1
                        if pred_idx >= 0 and pred_idx < outputs1.logits.size(1):
                            dist1 = F.softmax(outputs1.logits[0, pred_idx, :], dim=-1)
                
                # Fallback to last token if labels logic fails
                if dist1 is None:
                    dist1 = F.softmax(outputs1.logits[0, -1, :], dim=-1)

                # 7. Get distribution for perturbed prompt (next token prediction)
                dist2 = F.softmax(outputs2.logits[0, -1, :], dim=-1)
                
                # 8. Compute KL loss
                kl_loss = F.kl_div(dist1.log(), dist2, reduction="batchmean")
                
                # 9. Final loss
                final_loss = ce_loss + 0.1 * kl_loss
                
                # 9. Backward and return (scaled for gradient accumulation)
                if self.args.gradient_accumulation_steps > 1:
                    final_loss = final_loss / self.args.gradient_accumulation_steps
            ...
            print(f"[SA-PAFT] training_step error: {e}")
            return super().training_step(model, inputs)
